...
export(float,1.0,1.5) var MAX_DIAGONAL_SLOPE = 1.5  # Максимум углового коофициента
...
func _end_detection(position):
	...
	if (pointsDistance.x < -tapRadius or pointsDistance.x > tapRadius) or (pointsDistance.y <-tapRadius or pointsDistance.y > tapRadius):
	# Проверка на то чтобы расстояние между началом и концом свайпа было не больше tapRadius
		if abs(direction.x) + abs(direction.y) >= MAX_DIAGONAL_SLOPE:
		# Если модуль x + y больше максимума углового коофициента
			return
		if abs(direction.x) > abs(direction.y): # Если модуль X больше модуля Y
			emit_signal("swiped", Vector2(sign(direction.x),0.0))
			# Отправляет сигнал направления X
			# Функция sign() возвращает знак числа (Пример: sign(-6) вернет -1, sign(5) вернет 1).
		else:
			emit_signal("swiped", Vector2(0.0,sign(direction.y))) # Отправляет сигнал направления Y
	else: # Если расстояние между началом свайпа и его концом меньше tapRadius
		emit_signal("tap")
